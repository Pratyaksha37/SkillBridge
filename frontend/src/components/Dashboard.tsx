...
import { Search, Plus, Compass, Terminal, CircleDollarSign, Brush, Star, Handshake } from 'lucide-react';

export default function Dashboard() {
  return (
    <main className="max-w-7xl mx-auto px-8 py-12">
      <header className="mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-on-surface mb-2">Welcome back, Alex.</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            <StatBox value="24" label="Hours Taught" color="primary" />
            <StatBox value="12" label="Hours Learned" color="tertiary" />
            <StatBox value="4.9" label="Teacher Rating" color="secondary" />
          </div>
        </div>
...
function MatchCard({ name, role, matchRate, offers, wants, imageUrl, secondary = false }: MatchCardProps) {
  return (
    <div className="bg-surface-container-lowest p-6 rounded-[1.5rem] ambient-shadow border border-white relative overflow-hidden group">
      ...
      <button className={`w-full py-3 rounded-xl font-bold text-sm tracking-wide transition-all ${
        secondary 
